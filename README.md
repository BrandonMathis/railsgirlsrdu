...
The first thing we need to create to have Tweets in our application is a Model called Tweet. We can use this Model to create, update, and destroy Tweets with commands like `Tweet.create`, `Tweet.update_attributes`, and `Tweet.destroy`. Take a look in your `app/models` directory. You should see a newly created file called `tweet.rb` with the following contents
...
The next thing we need to do is update our database so we can store our tweets. To do this you must create a [Migration](http://guides.rubyonrails.org/migrations.html). Look inside your `db/migrations` directory. You will se a file that looks something like `20130908173312_create_tweets.rb`. Your file's name may not be the same but the contents will be.
...
This code was generated by the `rails generate model` command we ran earlier. It will modify our database and create a table called 'tweets'. Lets run it to create that table. Type the following into your command prompt.
...
####Controller/Views
Next we are going to do some front-end work to actually display tweets. In order to do that we are going to need to create some Tweet controllers and views. Controllers pull data out of the database like tweet content and serve that content up to views which render the HTML that is displayed to the users. Lets start with our controllers.
...
Inside this controller we are going to create 3 actions `index`, `create`, and `new`. To do this we need to setup a new route in our application. Add the following line bellow `Twitter::Application.routes.draw do`.
...
Index will be a list of every tweet that every user makes. Getting all these tweets is easy! Add the following code to your `tweets_controller.rb`
...
Nex up we will want to display all these tweets in a list on our homepage. We will use a view to do this. Create a file at `app/views/tweets` called `index.html.erb`. If the tweets directory doesn't exists, make it.
...
The index action will automatically look for a view at this location called index. Go ahead and start your rails server.

```
rails s
```

Finally, open localhost:3000/tweets in your browser and you should see an empty page with 'Tweets' heading at the top.
...
Now, you can go back to your application at localhost:3000/tweets (remember `rails s`!) and the "New Tweet" link should redirect you to a login page. From there, click "Sign up" to create a new user account (link under login form). Once you have a new user created, log that user in and click "New Tweet" at localhost:3000/tweets to create a new tweet. Craft your tweet and post it! Just like that we have the basics down for signup, login, and tweet posting!